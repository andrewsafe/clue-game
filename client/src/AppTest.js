...
function App() {
  ...
  const handleMovePlayer = () => {
    ...
    const handleMoveOptionsResponse = (data) => {
      if (data.error) {
        ...
        const options = data.options.map((option) => option[1]);
        const indexes = data.options.map((option) => option[0]);
        setMessage(
          `You are currently at ${data.currentLocation
          }. Available moves: ${indexes.join(", ")}`
        );
  ...
  const handleMove = () => {
    socket.emit("move_character", { move });

    const handleMoveResponse = (data) => {
      if (data.error) {
        console.error(`Error moving player ${move}:`, data.error);
        setMessage("An error occurred: " + data.error);
      } else {
      ...
      socket.off("move_character_response", handleMoveResponse);
    };

    socket.on("move_character_response", handleMoveResponse);
  };
